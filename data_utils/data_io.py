...
class SBU:
    ...
    def __init__(self, dirpath, shuffle=utils.SHUFFLE, mode=None):
        ...
        if mode is None:
            mode = utils.MODE
        self.mode = mode
        self.root_data_dir = dirpath + os.sep + train_dir_map[mode]
        print('---> Loading Dataset from', self.root_data_dir)
        self.images_dir_prefix = os.sep.join([self.root_data_dir, 'images'])
        self.labels_dir_prefix = os.sep.join([self.root_data_dir, 'labels'])
        self.im_fpaths = np.array(glob(self.images_dir_prefix + os.sep + '*'))
        self.shuffle = shuffle
        if shuffle:
            np.random.shuffle(self.im_fpaths)
        im_ext = '.' + self.im_fpaths[0].split('.')[-1]
        self.im_ids = np.array([fp.split(os.sep)[-1].replace(im_ext, '') for fp in self.im_fpaths])
        self.mask_fpaths = np.array([self.labels_dir_prefix + os.sep + id + '.png' for id in self.im_ids])
    ...
    def get_label(self, idx):
        mask = cv2.imread(self.mask_fpaths[idx])[:, :, 0]
        return mask

    def get_image(self, idx):
        im = cv2.imread(self.im_fpaths[idx])
        return im
    ...
    def preprocess(self, im_in, mask_in):
        ...
        if random_crop:
            ...
            mask = cv2.resize(mask[start_h:end_h, start_w:end_w].astype(np.float32),
                              (1280, 1280), interpolation=cv2.INTER_NEAREST)
        else:
